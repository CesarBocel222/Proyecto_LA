    ...
    public class ArbolExpresionD
    {
        ...
        public void GenerateDFA()
        {
            ...
            foreach (var clave in orden)
            {
                ...
                foreach (var elemento1 in FollowTable[clave]) Console.Write("{0} ", elemento1);
            }
            ...
            {
                aceptacion = estado.Contains(57);
                Console.Write("\n\n" +
                    "Estado: {");
                foreach (var num in estado)
                {
                    Console.Write("{0}, ", num);
                }
                Console.Write("}}, (aceptacion: {0}) :\n", aceptacion);
                foreach (var simbolo in DFATable[estado].Keys)
                {
                    Console.Write("Con simbolo: {0} , transiciona a: {{", simbolo);
                    foreach (var num in DFATable[estado][simbolo])
                    {
                        Console.Write("{0}, ", num);
                    }
                    Console.Write("}, ");
        ...
        public void writeNodeInfo(NodoM node)
        {
            ...
            foreach (var dato in node.First) Console.Write("{0}, ", dato);
            Console.Write("}, Last: {");
            foreach (var dato in node.Last) Console.Write("{0}, ", dato);
            Console.Write("}");

        }

        /// <summary>
        /// Aca almacenamos la cadena y su token Id
        /// </summary>
        private string SimboloAnt = "";
        private string SimboloAntDif = "";
        public void ActualizarInfoSimbolos(string simbolo, int tokenId)
        {
            if (SimboloAnt.Contains("|") && InfoSimbolos.Exists(x => x.SimboloId == tokenId))
            {
                if (simbolo.Length > 1)
                {
                    var simbInf = InfoSimbolos.Where(x => x.SimboloId == tokenId).FirstOrDefault();
                    var ultimaPos = simbInf.SimboloValor.LastIndexOf(SimboloAntDif);
                    var nuevaCadena = simbInf.SimboloValor.Substring(0, ultimaPos);
                    nuevaCadena += simbolo;
                    InfoSimbolos.Add(new infoVM { SimboloValor = nuevaCadena, SimboloId = tokenId });
                    SimboloAnt = simbolo;
                }
                else
                {
                    InfoSimbolos.Add(new infoVM { SimboloValor = simbolo, SimboloId = tokenId });
                }
                return;
            }

            if (simbolo != "|" && simbolo != "°")
                SimboloAntDif = simbolo;
            ...
            var index = InfoSimbolos.FindLastIndex(x => x.SimboloId == tokenId);
            if (index > -1)
                InfoSimbolos[index].SimboloValor += simbolo;
            else
                InfoSimbolos.Add(new infoVM { SimboloValor = simbolo, SimboloId = tokenId });
        }
